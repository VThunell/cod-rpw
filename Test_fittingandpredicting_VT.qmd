...
table(df$pred_weight_source)

df |>
  group_by(year, pred_weight_source) |>
  mutate(obs_est_rat = pred_weight_source)

df |>
  count(pred_weight_source, year) |>
  group_by(year) |>       
...
Model with famliy = log normal  
```{r log normal}
# hist(log(df_noz$fr_tot), breaks = 50 )
# 
# # make mesh
# mesh_noz <- make_mesh(df_noz, c("X", "Y"), cutoff = 6)
# plot(mesh_noz)
# 
# tot_space_ln <- sdmTMB(
...
# qqnorm(residuals(tot_space_ln))
# qqline(residuals(tot_space_ln))
# qqnorm(residuals(tot_space_ln, "mle-mcmc", mcmc_samples = space_res))
# qqline(residuals(tot_space_ln, "mle-mcmc", mcmc_samples = space_res))
# 
...
# # tidy(tot_space, conf.int = TRUE)
# # tidy(tot_space, effects= "ran_pars", conf.int = TRUE)
# # predict(tot_space)
# 
# space_res_g <- mcmc_res <- residuals(tot_space_g, type = "mle-mcmc", mcmc_samples =  sdmTMBextra::predict_mle_mcmc(tot_space_g, mcmc_iter = 201, mcmc_warmup = 200))
# 
# # Residuals
# qqnorm(residuals(tot_space_g))
# qqline(residuals(tot_space_g))
# qqnorm(residuals(tot_space_g, "mle-mcmc", mcmc_samples = space_res))
# qqline(residuals(tot_space_g, "mle-mcmc", mcmc_samples = space_res))
# 
# # Gamma semms more plausible given the data and has better qq-plot than log-normal for the abive model
...
# reduce numerb of years to reduce fitting time
df_tot_sel <- df_tot |> filter( Year %in% c(1980:2001))
mesh_nozdoy_sel <- make_mesh(df_tot_sel, c("X", "Y"), cutoff = 6)
...
saveRDS(M1f, file = paste("lammska_M1f_",date,".rds"))
...
```{r M1f}
M1f <- readRDS(M1f, file = paste("lammska_M1f_",date,".rds"))

# Conditional effects of predator length varying over time
nd_M1f_length <- expand.grid(
  pred_length_sc = seq(min(df_tot_sel$pred_length_sc), max(df_tot_sel$pred_length_sc), length.out = 50),
  Year = unique(df_tot_sel$Year))
...
nd_M1f_doy <- data.frame(Year = 1991, depth_sc = 0, pred_length_sc = 0, doy_sc = seq(min(df_tot_sel$doy_sc), max(df_tot_sel$doy_sc), length.out = 50))

p_M1f_doy <- predict(M1f, newdata = nd_M1f_doy, se_fit = TRUE, re_form = NA)


# Conditional effects of Year
nd_M1f_Year <- data.frame(Year = seq(min(df_tot_sel$Year), max(df_tot_sel$Year)), depth_sc = 0, pred_length_sc = 0, doy_sc = 0)
...
saveRDS(M1g, file = paste("lammska_M1g_",date,".rds"))
...
```{r}
M1g <- readRDS(M1g, file = paste("lammska_M1g_",date,".rds"))
...
# Conditional effects of day of the year
nd_M1g_doy <- data.frame(Year = 1991, depth_sc = 0, pred_length_sc = 0, doy_sc = seq(min(df_tot_sel$doy_sc), max(df_tot_sel$doy_sc), length.out = 50))

p_M1g_doy <- predict(M1g, newdata = nd_M1g_doy, se_fit = TRUE, re_form = NA)


# Conditional effects of Year
nd_Year <- data.frame(Year = seq(min(df_tot_sel$Year), max(df_tot_sel$Year)), depth_sc = 0, pred_length_sc = 0, doy_sc = 0)
...
saveRDS(M1h, file = paste("lammska_M1h_",date,".rds"))

```
Conditional effects for M1h

```{r}
M1h <- readRDS(M1h, file = paste("lammska_M1h_",date,".rds"))
...
p_M1h_length <- predict(M1h, newdata = nd_M1h_length, se_fit = TRUE, re_form = NA)
  ...
  # Conditional effects of day of the year
nd_M1h_doy <- data.frame(Year = 1991, depth_sc = 0, pred_length_sc = 0, doy_sc = seq(min(df_tot_sel$doy_sc), max(df_tot_sel$doy_sc), length.out = 50))
...
#  sdmTMB( 
#   data = df_tot_s,
#   formula = fr_tot ~ 0 + depth_sc + s(doy_sc, bs = "cc"), #+ pred_length_sc, #+ herring_sc + sprat_sc + saduria_sc + other_sc + benth_fish_sc + other_pelag_fish_sc + other_invert_sc, # 0 means no overall intercept
#   #time_varying = ~1*pred_length_sc, # implies time varying random walk intercept. # first*second indicates the cross of first and second. This is the same as first + second + first:second. 
#   extra_time = missing_years, # to fill in empty years
#   time = "Year", # for spatiotemporal 
#   mesh = mesh_nozdoy_s,
#   #spatiotemporal = "ar1",
...
df_tot_s <- slice_sample(df_tot, n = 80000) 

my <- min(df_tot$Year):max(df_tot$Year)
missing_years <- my[!my %in% unique(df_tot$Year)] 

# make mesh
mesh_nozdoy_s <- make_mesh(df_tot_s, c("X", "Y"), cutoff = 6)

# interaction model between length and time. We need
```
...
M1e_res2 <- sdmTMBextra::predict_mle_mcmc(M1e, mcmc_warmup = 150, mcmc_iter = 151)

```


```{r}
qqnorm(residuals(M1d))
qqline(residuals(M1d))
qqnorm(residuals(M1d, "mle-mcmc", mcmc_samples = m1d_res2))
qqline(residuals(M1d, "mle-mcmc", mcmc_samples = m1d_res2))


#library(ggeffects)
plot(ggeffect(M1d,  "depth_sc"))

nd_Year <- expand_grid(Year = seq(min(df_tot_s$Year), max(df_tot_s$Year), by = 3), pred_length_sc = seq(min(df_tot_s$pred_length_sc), max(df_tot_s$pred_length_sc)))
nd_Year$depth_sc <- 0
nd_Year$doy_sc <- 100

pr_Year <- predict(M1d, newdata = nd_Year, re_form = NA, se_fit = TRUE)
...
z <- chull(df_tot_s$X, df_tot_s$Y) # z are indices of the coordinates

coords <- cbind(df_tot$X[z], df_tot_s$Y[z])
...
pred_grid <- expand.grid(
  X = seq(min(df_tot_s$X), max(df_tot_s$X), cell_width),
  Y = seq(min(df_tot_s$Y), max(df_tot_s$Y), cell_width),
  year = c(1963:2021)
  )
