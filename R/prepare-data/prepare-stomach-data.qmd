...
#          common_prey_name = ifelse(!AphiaIDPrey %in% c(293743, 1625944, 126425, 322683, 236448, 126417, 293511, 119034), "other", common_prey_name)) # TODO Clupeidea can be both herring and sprat but I choose it to be other for now. 
...
agg_prey <- d |> 
  mutate(prey_class = NA,
         ...
         prey_class = ifelse(AphiaIDPrey %in% c(125464, 126752, 125716, 126505, 126736, 125516), "other_pelag_fish", prey_class),
         prey_class = ifelse(!prey_class %in% c("other_invert", "saduria", "benth_fish", "herring", "sprat", "other_pelag_fish"), "other", prey_class)) # NOTE, "clupea" and"clupeidea" etc. are "other"

unique(agg_prey$prey_class)
...
t <- agg_prey_summed |>
  select("other_invert", "benth_fish", "other_pelag_fish", "sprat", "herring", "other") |>
  rowSums() == 0
which(t) # empty stomachs..
```
...
dd <- dd |> 
  mutate(other = replace_na(other, 0),
         ...
         other_pelag_fish = replace_na(other_pelag_fish, 0))
...
missing_db3 <- missing_db3 |>
  mutate(prey_class = NA, 
         ...
         prey_class = ifelse( Prey_LatinName %in% c("Gasterosteus aculeatus", "Osmerus eperlanus", "Belone belone", "Engraulis encrasicolus"), "other_pelag_fish", prey_class),
         prey_class = ifelse(!prey_class %in% c("other_invert", "saduria", "benth_fish", "herring", "sprat", "other_pelag_fish"), "other", prey_class)) # NOTE, "clupea" and"clupeidea" etc. are "other"
...
unique(missing_db3$prey_class)
mm |> select("other", "other_invert", "saduria", "benth_fish","herring", "sprat", "other_pelag_fish") |>
  rowSums() == 0
...
# TODO THESE prey_classes ARE NOT fully compatible with the old db and new db data.
ll <- SE |>
  drop_na(group) |>
  drop_na(oxy) |>
...
newold <- newold |> 
  ...
  mutate(#fr_sad = saduria / pred_weight,
        ...
        # fr_other = other / pred_weight,
          fr_tot = sum(other, other_invert, benth_fish, other_pelag_fish, saduria, sprat, herring) / pred_weight)
...
```{r}
newold |> 
  pivot_longer(c("herring", "saduria", "sprat","other","other_pelag_fish","benth_fish")) |> 
  group_by(name) |> 
...
df <- newold |> 
  ...
  dplyr::select(pred_ID, herring, saduria, sprat, other, other_pelag_fish, benth_fish, fr_tot, Year, Month, day_of_year, pred_weight, pred_length, saduria, sprat, herring, lat, lon, ices_rect, X, Y)#, depth)
