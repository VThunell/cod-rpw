...
ll <- SE |>
  ...
  mutate(other = amphipoda_tot + bivalvia_tot + gadus_morhua_tot +  gobiidae_tot + mysidae_tot + non_bio_tot +
         other_crustacea_tot + other_tot + other_pisces_tot + platichthys_flesus_tot + polychaeta_tot)
...
new_SE_data$Day <- as.character(lubridate::yday(new_SE_data$date))
...
# Check if any of the two datasets have NA in the common columns. 
unique(is.na(old_db_data |> dplyr::select(all_of(comcol_newold_pred)))) # depth and time are all NAs and HaulNO we dont need
...
newold <- full_join( old_db_data |> dplyr::select(-c(Time, Depth, HaulNo)), # join in predator data
                     new_db_data |> dplyr::select(-c(Time, Depth, HaulNo)),
                     by = comcol_newold_pred[!comcol_newold_pred %in% c("Time","Depth","HaulNo")])  |>
  full_join(new_SE_data, by = comcol_newold_pred[!comcol_newold_pred %in% c("Time","Depth","HaulNo")] )
...
df <- newold |> 
  ...
  dplyr::select(fr_sad, fr_spr, fr_her, fr_oth, fr_tot, Year, Month, pred_weight, pred_length, saduria, sprat, herring, lat, lon, ices_rect, X, Y)#, depth)
