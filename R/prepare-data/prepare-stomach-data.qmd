...
# }
# 
# a <- read_ICES_format(data_dir = paste0(home, "/data/stomach/StomachContent_0115213707")) |> as.data.frame()
# a
# problems(a)
```
...
# I need to rename "Notes" in prey data to avoid confusion as to which dataset it belongs to.
prey <- prey |> rename(prey_notes = Notes) |> mutate(fish_id = paste(Year, Month, HaulNo, Day, FishID, sep = "_"))

# VT: Why do we need a globally unique predator ID when the ICES database download provides the ´tblPredatorInformationID´ which is a unique identifier. Creating thi new identifier did hwoever reveal that there are likely duplicates in the data.
...
# The fish_id is shorter by 48 fish. Some (37) of these fish are be duplicates with respect to all columns but 'tblPredatorInformationID'. Even their weights are the same. Remove these duplicates:
dups <- pred |> select(-tblPredatorInformationID) |> duplicated() |> which()
pred <- pred[-dups,]

# Remove regurgitated stomachs! (approx. 4%)
# TODOne?VT: in the Swedish data, 2 means regurgitated and 1 is intact, but for the rest, 1 means regurgitated, 0 or NA means intact...
pred <- pred |>
...
dtest <- left_join(prey, pred, by = c(comcol_prey_pred, "fish_id"))
...
prey |> filter(tblPredatorInformationID %in% c(regurg_ids$ƒPredatorInformationID)) |> distinct(Weight, AphiaIDPrey) |> as.data.frame()
...
# OK... 
# The pred_in_prey_not_pred are the regurgitated stomachs, which I filter away. If I don't filter away these, there are no predators that only exist in the prey file
...
# I guess this (VT: What -999?) is because they couldn't be counted only weighed
...
```{r}
#VT: no zero weights in the data from ices 15/1 2024!
...
# TODO: Is it really so that we should have prey info but no info on count or length or weight?? VT: well yes there can be body parts that tell what species but gives no info on count, weight or length. 
...
# Of course there are obvious issues....  VT: what are the issues?
prey_avg_ind_weight |>
  ggplot(aes(ind_weight)) +
  facet_wrap(~common_prey_name, ncol = 3, scales = "free") +
  geom_histogram()

#VT: Saduria is way off, not counted correctly? how to deal w those outliers.
prey_avg_ind_weight |>
  ggplot(aes(Weight)) +
  facet_wrap(~common_prey_name, ncol = 3, scales = "free") +
  geom_histogram()
...
# TODO: really, can't be 0 count or 0 length or 0 weight... Has to be NA comon and VT thinks the average saduria weight is too large.
```
...
ggplot(dd, aes(pred_length, IndWgt, shape = pred_weight_source)) + 
  geom_point()
```
...
plot_map_fc +
  geom_point(data = dd, aes(X*1000, Y*1000, color = depth), alpha = 0.5) +
  theme_sleek(base_size = 6) + 
  geom_sf()

ggplot(dd, aes(lon, lat, color = depth)) + 
  geom_point() + 
  coord_sf()
```
