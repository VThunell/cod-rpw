...
```{r quick glimpse and select old data}

# library("fisheryO")
# area_definition(ecoregion = "Baltic Sea Ecoregion")
glimpse(old_db)
# each row contains a prey item, i.e. we have many rows for each predator
# How to generate and identifier so that we can match the old and new data 
# ICESsampleID is for the predator i assume, ICESitmeID are prey items and there are duplicates, possibly mainly due to the two two data sets (Year of stomach and Stomach tender d using the same IDs which become duplicates) 
# By creating an ID with Date (Year, Month and Day), Country and Haul (Haul and HaulNo) we can identify the data missing from the new database data.
#unique(prey$Ship)
#unique(old_db$Ship)
# unique(old_db$Country)
#The Ship onfo in old data is unclean and hard to use, By instead assuming that Country should be unique to date and haul (sort of Country as a proxy for Ship). However, there are 41506 rows where country and ship is missing. VT removes these. 
#length(is.na(old_db[which(is.na(old_db$Country)),]$Ship))
# CSNAsh <- which(is.na(old_db$Country) & is.na(old_db$Ship))
# old_db <- old_db[-CSNAsh,]
# missing_dbprey |>
#   group_by(HaulID) |> 
#   #select(`Predator_Weight(mean)`) |> 
#   duplicated() |> 
#   which() |> 
#  length()
...
length(which(is.na(old_data$Country)))

# join hi and fi to get ...
hi <- left_join(hi, fi, by = "tblUploadID")

# add Country and ICESrectangle to prey data from haul data and create identifier
prey <- prey |>
  left_join(select(hi, tblUploadID, Country, ICESrectangle), by = "tblUploadID", multiple = "any") |>
  mutate(Haul_ID = paste(Country, Year, Month, Day, HaulNo, ICESrectangle, sep = "_"))

# identify cod in hauls in the baltic not present in the new data 
missing_db <- old_data |>
  anti_join(prey, by = "Haul_ID") # return all rows from x (old_d) without a match in y (prey)

# fix predator lengths and weights
missing_db2 <- missing_db |> 
  rename(count_pred = Stomach_TotalNo,
...
missing_db2 <- missing_db2 |> 
  ...
  mutate(prey_length = ifelse( prey_length >= 0, `Predator_Lengh(mean)`, Prey_LowerLengthBound)) |>
  mutate(prey_weight_source = ifelse(is.na(pred_weight) | pred_weight <= 0 & count_prey >= 0 , "estimated", "observed")) |> 
  mutate(prey_weight = ifelse( is.na(pred_weight) | pred_weight <= 0, (0.01*pred_length^3)*count_prey, pred_weight))
    
missing_db2 |> # 97 722 unique predators ?
  filter( pred_weight > 0) |>
  filter( prey_weight > 0) |>
  distinct(`SampleNo(FishID)`, Haul_ID) 

missing_db2 <- missing_db2 |> # predator identifier
  mutate(pred_ID = paste(Haul_ID, `SampleNo(FishID)`, sep = "_"))
  

  
  
#Stefan has date+station+ship+rectangle+?+?predstorlek+preyvikt+? i.e. he uses the predator and prey weight combined with the haul ID to find unique stomachs. But since there are multiple prey items for each predator, this will create This is not bullet preoof but is likely ok. For this I need length to weight

# Explore Stefan data..
unique(pred$Coun_)
# missing_dbprey2 <- missing_dbprey |> 
#   rowwise() |>
#   mutate( preds = sum(`Predator_Weight(mean)`, `Predator_Lengh(mean)`, na.rm = TRUE)) |> 
#   mutate( preys = sum(Prey_Weight, Prey_Length, na.rm = TRUE)) |> 
#   filter(preds > 0)# & preys > 0)
# 
# 
# #length(which(is.na(missing_dbprey$`SampleNo(FishID)`)))
# old_missingstomach <- missing_dbprey2 |>
#   select(`SampleNo(FishID)`, HaulID, Year) |>
#   #distinct(`SampleNo(FishID)`) |>
#   distinct(`SampleNo(FishID)`, HaulID) |>
  
  nrow() # 63050

old_missingstomach + nrow(pred) # 100000 or 134700 stomachs, c.f. Neuenfeldts 107069 rows. 

missing_dbprey |>
  select(`SampleNo(FishID)`, HaulID, `Predator_Lengh(mean)`) |>
  disti
  # group_by(HaulID) |>
...
# missing_dbprey <- missing_dbprey |>
#   add_utm_columns(ll_names = c("Longitude", "Latitude"), utm_crs = 32633) 
# plot_map +
#   geom_point(data = missing_dbprey, aes(X*1000, Y*1000))

missing <- missing_dbprey |>
  group_by(Year) |>
  distinct(`SampleNo(FishID)`) |>
  summarize(count = n())
sum(missing$count)

missing_dbprey$`Predator_Lengh(mean)`
length(which(is.na(missing_dbprey$ICES_ItemID)))  # and many rows without an identifier
length(which(is.na(missing_dbprey$`SampleNo(FishID)`))) # and lot s
#dups <- 
  missing_dbprey |> select(HaulID, ICES_SampleID, ICES_ItemID, `Predator_Weight(mean)`) |> duplicated() |> which()
#pred <- pred[-dups,]
...
group_by(Year) |>
  summarize(count = n()) |>
  ggplot() +  
  geom_bar(aes(Year, count), alpha = 1, stat="identity") +
  geom_bar(data = missing, aes(Year, count),  alpha = 0.5, stat="identity") +
  ggtitle("Grey bars is missing from new db") +
...
df <- dd |> 
  ...
  mutate(Month = as.character(Month),
         Day = as.character(Day),
         ices_subd = ices.rect2(lon, lat),) |> ##VT ??? 
  add_utm_columns(ll_names = c("lon", "lat"), utm_crs = 32633) |> 
