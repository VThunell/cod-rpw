...
agg_prey <- d |> # isopoda may cause issues
...
agg_prey |> filter(Weight <= 0) |> dplyr::select(prey_class, Weight, prey_length, Count)
...
t <- agg_prey_summed |>
  dplyr::select("other_invert", "benth_fish", "other_fish", "sprat", "herring", "other") |>
  rowSums() == 0
...
# No duplicates?
missing_db3 |> group_by(Haul_ID) |> dplyr::select(-Dataset, -ICES_SampleID) |> #ID, `Predator_Weight(mean)`) |> 
  duplicated() |> which()
...
mm |> dplyr::select("other", "other_invert", "saduria", "benth_fish","herring", "sprat", "other_fish") |>
  rowSums() == 0
...
SE_wide2 <- SE_wide2 |> # slpitting clupeids into equal parts sprat and herring
  mutate(sprat = sprat + unid_clupeids*0.5,
         herring = herring + unid_clupeids*0.5) |> 
  dplyr::select(!unid_clupeids)
...
# quantile(newold2$fr_tot, 0.99) 
# # remove feeding ratios above the 99th percentile 
newold3 <- newold2 #|>
#   filter(between(fr_tot, 0, 0.33)) |>
#   filter(between(fr_spr, 0, 0.33)) |> 
#   filter(between(fr_her, 0, 0.33)) |> 
#   filter(between(fr_sad, 0, 0.33)) |> 
#   filter(between(fr_other, 0, 0.33)) |> 
#   filter(between(fr_other_invert, 0, 0.33)) |> 
#   filter(between(fr_other_fish, 0, 0.33)) |> 
#   filter(between(fr_benth_fish, 0, 0.33))
  
newold3 |>
  dplyr::select(fr_sad, fr_spr, fr_her, fr_tot, fr_other, fr_other_invert, fr_benth_fish, fr_other_fish) |> 
  pivot_longer(everything()) |> 
  ggplot(aes(value)) +
...
df <- newold3 |> 
  ...
  dplyr::select(pred_ID, herring, saduria, sprat, other, other_invert, other_fish, benth_fish, fr_tot, fr_sad, fr_spr, fr_her, fr_other, fr_other_invert, fr_benth_fish, fr_other_fish, year, month, day_of_year, pred_weight, pred_length, pred_weight_source, saduria, sprat, herring, lat, lon, ices_rect, X, Y, data_source, Country)#, depth)
