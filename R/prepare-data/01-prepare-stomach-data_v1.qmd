...
pkgs <- c("tidyverse", "tidylog", "devtools", "sdmTMB", "RCurl", "purrr", "janitor", "patchwork", "mapplots", "worrms", "fuzzyjoin", "rfishbase", "furrr")
...
new_dat3 <- new_dat2 |> filter(AphiaIDPredator %in% c(126436))

ggplot(new_dat3, aes(pred_length, pred_weight)) + # 0, negative or NAs weight with lengths
  geom_point()

# Estimate predator weight if we have their length? get cod a and b
# preyTaxamatch_LW |>
#   filter(scientificname == "Gadus morhua") |>
#   dplyr::select(a,b) |> distinct()

new_data <- new_dat3 |> 
  mutate(pred_weight = replace_na(pred_weight, 0),
         pred_weight_source = ifelse(pred_weight <= 0, "estimated", "observed"),
         pred_weight = ifelse(pred_weight_source == "estimated", 0.00712575*pred_length^3.098685, pred_weight))		
...
# Fix predator weights from lengths
```{r}
# Estimate predator weight if we have their length? get cod a and b
# preyTaxamatch_LW |>
#   filter(scientificname == "Gadus morhua") |>
...
missing_db10_sum5 <- missing_db10_sum4 |> 
  mutate(pred_weight_source = if_else(is.na(pred_weight) | pred_weight <= 0, "estimated", "observed"),
         pred_weight = if_else(pred_weight_source == "estimated", 0.00712575*pred_length^3.098685, pred_weight, missing = NA)) 
...
missing_db10_sum5 |>
  summarise(n=n(), .by = pred_weight_source)
# TODO consider using baltic specific a and b. interseting that the new and old db differs in the lw-relationship.
...
old_data <- missing_db10_sum5 |> 
  dplyr::select(Country,Estimated_Lat_Lon,ICESrectangle,Year,Month,Day,pred_weight,pred_length,Haul_ID,lat,lon,pred_ID,other_inverts,other_chords,other,herring,sprat,saduria,pred_weight_source,Depth)
...
df <- all_data2 |> 
  ...
  dplyr::select(pred_ID, herring, saduria, sprat, other, other_inverts, other_chords, fr_tot, fr_sad, fr_spr, fr_her, fr_other, fr_other_inverts, fr_other_chords, year, month, day, day_of_year, pred_weight, pred_length, pred_weight_source, lat, lon, ices_rect, X, Y, data_source, Country, Depth)
