...
pkgs <- c("tidyverse", "tidylog", "devtools", "sdmTMB", "RCurl", "purrr", "janitor", "patchwork", "scales", "mapplots", "worrms", "fuzzyjoin", "rfishbase", "furrr", "broom", "mgcv", "tidygam")
...
## Prey phylogenetic summary from all cod stomach data

From the prey AphiaIDs and Scientific names in the different databases we create a full taxonmic list of Baltic cod prey species. We add length weight parameters where possible (a & b from literature, fishbase or estimated). We can then join in accepted Scientific names or AphiaIDs to all prey species in the dbs and then calculate weights from length or vice versa. So firts we want to use find uniqque AphiaIDs and Scientifc names and get valid_Scientific names for. 

```{r check ODB for name error}
# where we have both AphiaID and Scientificname, we can see whether these match using get name from aphia ID. The problem is that species may be wrongly idd but its more likely however that just the AID is wrong I think. The AID should come after the name I mean in the identification-enter in db-process. 
...
# For these I think we need to skip the AID and go for Names instead. NAmes shold be more reliable I think when entering species. So if both Prey_LatinName and Prey_AphiaID is present, we use name for gettign records and LW as and bs.

# alternatively we can get names for all AID:s in the ODB (more than 300 000) and then remove the data where names doesnt match. But, many AIDs are e.g. genus och fanily f a species and is still sort of correct.-
...
# 
# # Remove AphiaIDs in the ODB where both scientific name and AID exists since its likely wrong (as many Scientific name does not macth AphiaIDs) and scientific name is more likely correct. By doing this, we remove instances where the db_ScientificName is way off the correct AID.
# ODB_b <- ODB|>
...
# these are for mm while fishbase is for cm. We´ll correcct for this below
...
#          !is.na(valid_name)
#          ) |>
#   stringdist_left_join(NSab |> dplyr::select("Species", "a", "b"), by = c(valid_name = "Species")) |>
...
#     filter(Type == "TL") |> # filter out total length messurements
...
#   pull(valid_name) |>
#  future_imap(\(x, idx) getabsChord(x, idx)) |>
#  list_rbind()
# plan(sequential)
# 
# preyTaxamatch_Chord <- preyTaxamatch_Chord |>
...
preyTaxamatch_LW |>
  ...
  ggplot() +
  geom_histogram(aes(value, color = abTaxlevel), bins = 200) +
  facet_wrap(params~abTaxlevel, nrow = 2)
...
#TODO: is it possible that there are both 2vs1 and 1vs0 in SE data, i.e. two codes for regurging in SE. # check what is most common and with Carolina. But it doesnt seem like it
pred |>
  filter(Country == "SE") |>
...
new_db3 <- new_db2 |> 
  mutate(prey_length = if_else(UnitLngt == "mm" & prey_length >= 0, prey_length/10, prey_length, missing = NA), # make cm
         ...
         Count = ifelse(Count <= 0 & (prey_length > 0 | prey_weight > 0), 1, Count), # Count Na or 0 should be 1 when there is a weight or length.
         prey_weight_source = if_else(prey_weight <= 0 & prey_length > 0 & Count > 0, "estimated", "observed", missing = NA), # NA lenght are ghetting wiegths
...
new_db4 <- new_db3 |>
  ...
  #filter(prey_weight <= 0 & Count > 0 & !is.na(AphiaIDPrey)) |>
...
# Negative weights (127 rows, <1%) and the NAs ( rows, which are caused by NAs or lacking weight for an unusual prey)
new_db4 |> filter(prey_weight <= 0) |> as.data.frame() |> distinct(prey_weight, prey_length, Count)

# Remove these (nearly 12000 rows!) and these are mainly the empties but also 270 na count/weight/length in prey.
new_db4 <- new_db4 |>
...
new_db4 |> # We split the non idd clupeids into sprat and herring below
...
new_db5 <- new_db4 |> # arguments are evaluated in order so proceed from specific to general
  mutate(prey_group = case_when(valid_name == "Clupea harengus"~ "herring",
                                ...
                                !phylum == "Chordata" & !is.na(AphiaIDPrey) ~ "other_inverts",
                                .default = NA))

# seems to work
new_db5 |>
  summarise(n = n(), .by= prey_group)
new_db5 |> # only NA prey ids in the other group
...
ggplot(new_dat, aes(pred_weight)) + 
...
new_dat2 |> # Note the one data point in 1989
...
# we get rid of almost all NAs (198 are left) in Haul by excluding based on lat lon (Country not needed then)
...
Fix prey lengths and weights and add taxonomy in the odb.
...
missing_db9 <- missing_db8 |> # arguments are evaluated in order so proceed from specific to general
  mutate(prey_group = case_when(valid_name == "Clupea harengus"~ "herring",
                                ...
                                phylum == "Chordata"~ "other_chords",
                                !phylum == "Chordata" & !is.na(valid_name) ~ "other_inverts",
                                .default = NA))
...
# In the old db (which is not the case for the new db), we have both pred and prey data. We can then use mutate() and distinct() instead of summarise(). 
...
old_data |> # there are unlikely extremes. These are filtered on feeding ratio in the analysis.
  ...
  ungroup() |> 
  summarise(empty_rat = n(), .by = empty_stom) # 35 000 empty stomachs... 90 000 not empty
...
# Estimate weight of predators from length using yearly a:s and b:s
The size of the cod in both the new db and old db are mainly represented by lengh and the length-weight relationship in eastern baltic cod has varied over time. This will likely affect our feeding ratio estimates and we want to account for this by estimate a yearly a and b using combined data from BITS (1978-) and the stomach data (1963-). 
...
# problems(read_csv(paste0(home, "/data/clean/lw_data.csv")))
lw_data_bits <- read_csv(paste0(home, "/data/clean/lw_data.csv"), 
                         col_types = list(
                           Year = col_double(),
                           SD = col_character(),
                           LngtCm  = col_double(),
...
# estimate weight based on year a and b and on fishbase a and b for comparison. 
all_data4 <- all_data3 |>
...
# make this a nicer plot, generate some data and 
all_data4 |>
...
all_data5 <- all_data4 |> 
  ...
  mutate(fr_sad = saduria / pred_weight,
         ...
         fr_other_inverts = other_inverts / pred_weight,
         fr_other_chords = other_chords / pred_weight,
         fr_tot = sum(other, other_inverts, other_chords, saduria, sprat, herring) / pred_weight,
         fr_tot_fb = sum(other, other_inverts, other_chords, saduria, sprat, herring) / pred_weight_fb) |>
  filter(between(fr_tot, 0, 1 )) |> ungroup()

all_data5 |>  # log so we can see the data better
  dplyr::select(fr_sad, fr_spr, fr_her, fr_tot, fr_other, fr_other_chords, fr_other_inverts) |> 
  #dplyr::select(fr_tot) |> 
...
df <- all_data6 |> 
  ...
  dplyr::select(pred_ID, herring, saduria, sprat, other, other_inverts, other_chords, fr_tot, fr_sad, fr_spr, fr_her, fr_other, fr_other_inverts, fr_other_chords, year, month, day, day_of_year, pred_weight, pred_weight_fb, pred_length, pred_weight_source, lat, lon, ices_rect, X, Y, data_source, Country, Depth)
...
write_csv(df, paste0(home, "/data/clean/stomachs5.csv")) # StomachContent_0115213707
#write_csv(df, paste0(home, "/data/clean/stomachs4.csv")) # StomachContent_0820100197 
```

```{r}
# check for round gobies over time
# AID 126916 
# bind_rows(new_db5 |> filter(valid_AphiaID == 126916) |> summarise(count = n(), .by = Year),
#           missing_db9 |> filter(valid_AphiaID == 126916) |> summarise(count = n(), .by = Year),
#           SE4 |> filter(valid_AphiaID == 126916) |> summarise(count = n(), .by = year) |> rename(Year = year)) |>
#   arrange(Year) |>
#   ggplot() +
#   geom_bar(aes(Year, count), stat="identity") +
#   scale_x_continuous(n.breaks = 10) + ylab("# N. melanostomus")
# 
# bind_rows(new_db5 |> filter(family == "Gobiidae") |> summarise(count = n(), .by = Year),
#           missing_db9 |> filter(family == "Gobiidae") |> summarise(count = n(), .by = Year),
#           SE4 |> filter(family == "Gobiidae") |> summarise(count = n(), .by = year) |> rename(Year = year)) |>
#   arrange(Year) |>
#   ggplot() +
#   geom_bar(aes(Year, count), stat="identity") +
#   scale_x_continuous(n.breaks = 10) + ylab("# Gobiidae")
