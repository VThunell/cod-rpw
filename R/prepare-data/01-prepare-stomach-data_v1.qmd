...
# New data (from 20 August) that contains less stomachs: StomachContent_0820100197 
# New data from January 2024: StomachContent_0115213707
...
#pred <- read_csv(paste0(home, "/data/stomach/StomachContent_0820100197/PredatorInformation.csv")) # Not edited
...
## Create prey species phylogenetic summary from all cod stomach data

From the prey AphiaIDs and Scientific names in the different databases we create a full taxonmic list of Baltic cod prey species. We add length weight parameters where possible (a & b from literature or estimated). We can then fuzzy join in accepted AphiaIDs to all prey species in the dbs using AphiaIDs or Scientific names and then calculate weights from length or vice versa. Names in the db that wont be fuzzy joined, we have to correct manually.
...
# Collect all AphiaIDs and latin names respectively and clean names
# Start with aphiaIDs
aphias <- 
  prey |> filter(AphiaIDPredator == 126436) |> dplyr::select(AphiaIDPrey) |> filter(!is.na(AphiaIDPrey)) |>
  bind_rows(ODB |> filter(Predator_LatinName == "Gadus morhua") |> dplyr::select(Prey_AphiaID) |> rename(AphiaIDPrey  = Prey_AphiaID) |> filter(!is.na(AphiaIDPrey))) |>
  distinct() |>
  rename(db_AphiaID = AphiaIDPrey)

# Function for getting Scientific names from AphiaIDs
getNamefAid <- function(aid)  {
  
    as_tibble(tryCatch(wm_id2name(id = aid),
  error = function(e){
        message("Aphia ID missing, returning NA:\n", e) 
    NA
         }
  )) |> bind_cols(db_AphiaID = aid)
  }

# apply function to aphias
aphias2 <- aphias$db_AphiaID |>
map(\(x) getNamefAid(x)) |>
list_rbind() |>
  rename(ScientificName = value)

# there are a few missing Aphia IDs. but these have their prey_latin_name in the ODB. Thereore, we can deal with those in the end along with other uniiid prey species
ODB |>
  filter(Prey_AphiaID %in% (aphias2 |> filter(is.na(ScientificName)) |> pull(db_AphiaID)) ) |>
  dplyr::select(Prey_LatinName) |>
  distinct() 

# This means we can remove these from the aphias
aphias2 <- aphias2 |> filter(!is.na(ScientificName))

# Now do Scientific names
latnames <- 
  SE |> filter(predator_code == "COD") |> dplyr::select(prey_latin_name) |> 
  bind_rows(ODB |> filter(Predator_LatinName == "Gadus morhua") |> dplyr::select(Prey_LatinName) |> rename(prey_latin_name = Prey_LatinName)) |>
  distinct() |> 
  mutate(ScientificName = make_clean_names(prey_latin_name), # some names cleaning her and below
         ScientificName = str_to_title(ScientificName),
         ScientificName = str_remove_all(ScientificName,"//."),
         ScientificName = str_remove(ScientificName,"2"),
         ScientificName = str_remove_all(ScientificName," "),
         ScientificName = str_replace_all(ScientificName,"_", " "),
         ScientificName = if_else(str_ends(ScientificName, "sp"), str_remove(ScientificName,"sp"), ScientificName),
         ScientificName = str_replace_all(ScientificName,"_", " "))

# filter out the latnames that we dont already have in aphias
unIDlatnames <- latnames |>
  filter(!ScientificName %in% aphias2$ScientificName) 

# Function for getting AphiaIDs from Scientific names
getAidfName <- function(nam, ind)  {
  
   as_tibble(tryCatch(wm_name2id(name = nam), error = function(e){
          message("Scientific name missing, returning NA:\n", e)
     NA
        }
   )) |> bind_cols(unIDlatnames[ind,]) # use index to save the db latin name, preserve this for matching in db
}

# apply function to latnames
latnames2 <- unIDlatnames$ScientificName |> ##
imap(\(x, idx) getAidfName(x, idx)) |>
list_rbind() |> 
  rename(db_AphiaID = value, db_ScientificName = prey_latin_name)

# combine all Scientific names (and their db synonyms) and AphiaIDs
preySpecies <- latnames2 |>
  bind_rows(aphias2)
...
getRecordsfAid <- function(aid, ind)  {
  
    as_tibble(tryCatch(wm_record(id = aid), error = function(e){
           message("Records missing, returning NA:\n", e) 
  NA
  })) |> bind_cols(preySpecies[ind,1:2])
}

# get records for the prey species list based on AphiaIDs (but remove NAs)
preySpecies_records <- preySpecies$db_AphiaID |>
  imap(\(x, xid) getRecordsfAid(x,xid) ) |>
      list_rbind()

#preySpecies_records2 <- preySpecies_records |> filter(if_all(everything(), ~ is.na(.)))

sum(is.na(preySpecies_records$valid_AphiaID)) # for these we lack AID and only have unmatched Scientific names
# get these 
unIDd_Scinames <- preySpecies_records |>
  filter(is.na(valid_AphiaID))
  #dplyr::select(db_ScientificName) |>
  #rename(invalid_SN = db_ScientificName)

# and see if fix these with fuzzy similar species names in the list
unIDd_Scinames_fuz <- unIDd_Scinames |> 
  filter(!is.na(db_ScientificName)) |> 
  dplyr::select(db_ScientificName) |>
  stringdist_left_join(preySpecies_records |> filter(!(is.na(db_AphiaID) | is.na(valid_name))) , by = c(db_ScientificName = "valid_name"))
# for the remaining non-id:d, we correct the organic animal prey (reomev plants and trash) manually
unIDd_Scinames_fuz |> filter(is.na(valid_AphiaID)) |> pull(db_ScientificName.x)

unIDd_Scinames_fixed <- unIDd_Scinames_fuz |> filter(is.na(valid_AphiaID)) |>
  mutate(valid_AphiaID = case_when(db_ScientificName.x == "Pisces/Osteichthyes" ~ 152352,
                                   db_ScientificName.x == "Gammarus sp."  ~  101537,
                                   db_ScientificName.x %in% c("Stone","Waste","Wood","Algae","stone","wood", "Aglae", "plastic", "carbon","Plastics","Litter/plastics", "Phaeophycophyta") ~ NA, 
                                    db_ScientificName.x %in% c("Mytilus sp.", "Mytilus sp") ~ 138228,  
                                    db_ScientificName.x == "Gobius niger" ~ 126892,
                                    db_ScientificName.x == "Idotea sp." ~ 118454,
                                    db_ScientificName.x == "Crangon" ~ 107007,
                                    db_ScientificName.x %in% c("remains","Remains","digestive tract") ~ NA,
                                    db_ScientificName.x == "Clupeoidei" ~ 125464,
                                    db_ScientificName.x == "Palaemon sp." ~ 107032,
                                    db_ScientificName.x == "Pectinaria sp." ~ 129437,
                                    db_ScientificName.x == "Pleuronectoidei" ~ 125579,
                                    db_ScientificName.x == "Corystes cassivelanus" ~ 107277,
                                    db_ScientificName.x == "Pectinaria sp." ~ 129437,
                                    db_ScientificName.x == "Sipunculoidea" ~ 1648,
                                    db_ScientificName.x == "Spirontocaris securifrons" ~ 107531,
                                    db_ScientificName.x == "Spirontocaris affinis" ~ 106994,
                                    db_ScientificName.x == "Ciliata mustella" ~ 126448,
                                    db_ScientificName.x == "Ophiothricidae" ~ 123208,
                                    db_ScientificName.x == "Leptocardia" ~ 104897,
                                    db_ScientificName.x == "Paleonemertea" ~ 122307,
                                    db_ScientificName.x == "Brachyrhyncha" ~ 736748,
                                    db_ScientificName.x == "Astarte procera" ~ 1735368,
                                    db_ScientificName.x == "Paleonemertea" ~ 122307,
                                    db_ScientificName.x == "Gadoidei" ~ 125469,
                                    db_ScientificName.x == "Macropipus dupurator" ~ 1750291,
                                    db_ScientificName.x == "Asterias rubens" ~ 123776,
                                    db_ScientificName.x == "Astropecten duplicatus" ~ 178650,
                                    db_ScientificName.x == "Amphitrite" ~ 204182,
                                    db_ScientificName.x == "ASTARTE" ~ 137683, 
                                    db_ScientificName.x == "Pecten raveneli" ~ 394070,
                                    db_ScientificName.x == "Phyllodoce" ~ 129455, 
                                    .default = NA),
         valid_AphiaID = as.integer(valid_AphiaID))
  

# get records for the fixed AIDs, reuse preySpecies as name for the df to use getRecordsAid() function
preySpecies <- unIDd_Scinames_fixed |> filter(!is.na(valid_AphiaID)) |> dplyr::select(valid_AphiaID, db_ScientificName.x)
new_preySpecies_records <- preySpecies |> pull(valid_AphiaID) |>
  imap(\(x, xid) getRecordsfAid(x,xid) ) |>
      list_rbind()

# check columsn before binding rows
janitor::compare_df_cols(preySpecies_records,new_preySpecies_records)
new_preySpecies_records |> 
  filter(!valid_AphiaID...9==valid_AphiaID...28)
         
preyTaxamatch <- new_preySpecies_records |> 
  mutate(valid_AphiaID = if_else(valid_AphiaID...9 == valid_AphiaID...28, valid_AphiaID...28, NA)) |>
  dplyr::select(-valid_AphiaID...9, -valid_AphiaID...28) |> # remove the old db_AphiaID columns
  rename(db_ScientificName = db_ScientificName.x) |>
  bind_rows(preySpecies_records) |> dplyr::select(-value)
...
#TODO: is it possible that there are both 2vs1 and 1vs0 in SE data, i.e. two codes for regurging in SE.
# check what is most common and with Carolina
pred <- pred |>
...
# There are regurgitated stomachs (column in pred) that have information in prey info, i.e. either its incorrect or they have signs of regurgitation but prey in the stomach. These are various prey types from 2018 to 2021. Lets remove these for the fr analyses
regurg_ids <- pred |>
...
new_db |> # we can live with the assumption that NA in Count equals 1 base on freq distribution of 1 and NA
  summarise(n= n(), .by = Count) |>
  arrange(Count)

# compare weight of count = 1 and count = NA

new_db |> # we can live with the assumption that NA in Count equals 1 base on freq distribution of 1 and NA. The lower mean weight is probably becasue the prey was not intact. 
  filter(is.na(Count) | Count == 1) |>
  mutate(Count = replace_na(Count, -1)) |>
  filter(Weight > 0)|>
  summarise(mean = mean(Weight), med = median(Weight),
            .by = Count)


# HERE
# TODO, get back to getteing specoes specific length-weight.
...
#          Count = ifelse(Count == 0, 1, Count), # Oteherwise we have zero weight when we have length. This does not affect weight estimates that could lead to inaccurate count estimates.
#          prey_length = replace_na(prey_length, -9), # Because there is both negative values and NAs
#          Count = if_else(prey_length > 0 & Count < 0, 1, Count, missing = -9),# When prey_length exists but Count <= 0 we set them to 1 (given the assumption above)
...
# ADD WHEN we have LW for saduria 
# new_t <- new_db |> 
#   mutate(Weight = if_else(prey_weight_source == "estimated", (0.01*prey_length^3)*Count, Weight, missing = -9),
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% c(293743, 1625944, 126425, 322683, 236448), (0.00575*prey_length^3.09)*Count, Weight, missing = -9), # sprat
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% c(126436), (0.0085*prey_length^3.02)*Count, Weight, missing = -9), # cod, mean a and b. https://github.com/maxlindmark/pred-prey-overlap/blob/main/R/prepare-data/01-collate-cpue-data-exchange.qmd#L502-L550
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% c(127141), (0.00575*prey_length^3.09)*Count, Weight, missing = -9), # flounder
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% 126417, prey == "herring", (0.00562*prey_length^3.09)*Count, Weight, missing = -9), # herring
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% c(293511, 119034), ((10*0.058*prey_length^3.052)/1000 * 0.179)*Count, Weight, missing = -9)) # Saduria. This gives DW in mg from length in mm! Convert to WW in g (1000) and cm (10)
#          Weight = if_else(prey_weight_source == "estimated" & AphiaIDPrey %in% c(293511, 119034), ((10*0.058*prey_length^3.052)/1000 * 0.179)*Count, Weight, missing = -9)) # Saduria.
...
# Nereididae and Petromyzontidae refuses to be rename
...
write_csv(df, paste0(home, "/data/clean/stomachs3.csv")) # StomachContent_0115213707
#write_csv(df, paste0(home, "/data/clean/stomachs4.csv")) # StomachContent_0820100197 
```
