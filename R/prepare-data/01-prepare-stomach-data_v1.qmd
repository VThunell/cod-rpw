...
The stomach content database for the Baltic sea (all countries, year 1963-2021) was downloaded at 16:21 Jan 15 2024.
...
```{r lw coeffs}
# Using NSab. Coefficients are for mm while fishbase is for cm. We´ll correcct for this below
#
...
preyTaxamatch_LW |>
  ...
  ggplot() +
  geom_histogram(aes(value, fill = abTaxlevel), legend = FALSE) + #, bins = 100) +
...
### Fix NDB pred regurgitated stomachs
...
missing_db10_sum3 |> # no ther isnt
  map(\(.) sum(is.na(.)))
...
## 9. Last fixes, spatiotemporal distribution and save data
