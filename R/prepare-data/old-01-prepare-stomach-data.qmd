...
agg_prey <- d_c |> # isopoda are likely often saduria but may also be other species. 
  mutate(prey_class = NA,
         ...
         prey_class = if_else(!prey_class %in% c("other_invert", "saduria", "benth_fish", "herring", "sprat", "other_fish", "unid_clupeids"), "other", prey_class)) # NOTE "other"
...
# Fixed but those NA latin names where Stomach_Item has a name ending with a space (" ") still show up in the summary:
missing_db4 |>
  filter(is.na(Prey_LatinName)) |>
  summarise(fix = unique(Stomach_Item))
...
missing_db4 <- missing_db4 |>
  mutate(prey_class = NA, 
         ...
         prey_class = if_else( Prey_LatinName %in% c("Gasterosteus aculeatus", "Osmerus eperlanus", "Belone belone", "Scombridea", "Salmonidae", "Pungitius pungitius", "Engraulis encrasicolus", "Petromyzontidae", "Syngnathus typhle"), "other_fish", prey_class),
         prey_class = if_else(!prey_class %in% c("other_invert", "saduria", "benth_fish", "herring", "unid_clupeids", "sprat", "other_fish"), "other", prey_class, missing = NA))
...
mm |> # there are unlikeley extremes. These are filtered on feeding ratio in the analysis.   
  rowwise() |>
  mutate(total = sum(other, other_invert, saduria, benth_fish, herring, sprat, other_fish)) |>
  filter( total > 1000) |>
  arrange(desc(total)) |>
  ungroup()
...
df <- newold2 |> 
  ...
  dplyr::select(pred_ID, herring, saduria, sprat, other, other_invert, other_fish, benth_fish, fr_tot, fr_sad, fr_spr, fr_her, fr_other, fr_other_invert, fr_benth_fish, fr_other_fish, year, month, day, day_of_year, pred_weight, pred_length, pred_weight_source, saduria, sprat, herring, lat, lon, ices_rect, X, Y, data_source, Country)#, depth)
...
write_csv(df, paste0(home, "/data/clean/stomachs2.csv"))
```
