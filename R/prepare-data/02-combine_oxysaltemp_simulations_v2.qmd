...
hind <- readRDS(file = paste0(home, "/data/environment/hindcast_1961_2017.rds")) |>
  mutate(#model = as.factor("hindcast"),
         year.month = paste(year, month, sep = ".")
         ) |>
...
# some cope points are outside the hind grid
wide |>
  filter(is.na(temp_n)) |>
  ggplot(aes(lon, lat)) +
  geom_point(size = 0.1)
...
### Oxygen 
```{r}
# rm(d_sub,env_sub,env_list, env_raster)
# gc()
oxy_dat <- wide |> 
  filter(year %in% c(2001:2017)) |> 
  mutate(diff = oxy - oxy_n) |> 
  drop_na(diff) |>
  select(diff, X, Y, year)

# Make mesh & fit models
mesh <- make_mesh(oxy_dat, cutoff = 15, xy_cols = c("X", "Y"))
plot(mesh)
