...
# 3rd quantile of the fr over time and pred length 
```
...
```{r predictions year using Mod_s}
...
pred_Mod <- predict(Mod_s, newdata = nd_pred_grid, return_tmb_object = TRUE) # TRUE returns a report from TMB in the output

pred_Mod_s <- predict(Mod_s, newdata = nd_pred_grid, nsim = 250) # måste ha TRUE här!!
