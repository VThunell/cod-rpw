---
title: "Feeding ratio total fit and effects"
...
bind_rows(y_Mod, y_s_Mod, y_n_Mod) |>
  ...
  geom_line() +
  geom_ribbon(aes(ymin = exp(coef.est-1.96*coef.se), ymax = exp(coef.est+1.96*coef.se)), color = NA, alpha = 0.3) +
  scale_x_continuous(breaks = seq(min(df_tot$year), max(df_tot$year), by = 10)) +
...
ggplot(aes(year, exp(coef.est), fill = model, color = model)) +
  geom_line() +
  geom_ribbon(aes(ymin = exp(coef.est-1.96*coef.se), ymax = exp(coef.est+1.96*coef.se)), color = NA, alpha = 0.3) +
  scale_x_continuous(breaks = seq(min(df_tot$year), max(df_tot$year), by = 10)) +
...
bind_rows(p_n_dsc, p_s_dsc, p_st_dsc) |> 
  ...
  geom_line() +
  geom_ribbon(aes(ymin = exp(est-1.96*est_se), ymax = exp(est+1.96*est_se)), color = NA, alpha = 0.3) +
...
cog_s <- #  roughly 1 hour proc time for the st model
  get_cog(
  pred_Mod_st,
  level = 0.95,
  format = "wide",
...
ggplot(cog_s, aes(mean_x, mean_y, color = decade )) +
#ggplot(cog_s, aes(est_x, est_y, color = year )) +
  geom_pointrange(aes(xmin = lwr_x, xmax = upr_x)) +
  geom_pointrange(aes(ymin = lwr_y, ymax = upr_y)) +
  scale_colour_viridis_c() +
  labs(title = "center of gravity") +
plot_map_fc +
   geom_pointrange(data = cog_s, aes(est_x*1000, est_y*1000, xmin = lwr_x*1000, xmax = upr_x*1000, colour = decade )) +
   geom_pointrange(data = cog_s, aes(est_x*1000, est_y*1000, ymin = lwr_y*1000, ymax = upr_y*1000, colour = decade )) +
...
pred_Mod_st$data <- pred_Mod_st$data |>
  ...
  mutate(dec_mean_est = mean(est)) |>
  mutate(dec_mean_enrf = mean(est_non_rf)) |>
  mutate(dec_mean_erf = mean(est_rf)) |>
  mutate(dec_mean_eps = mean(epsilon_st)) |>
  ungroup()

# estimate
plot_map_fc +
  geom_point(data = pred_Mod_st$data, aes(X*1000, Y*1000, color = exp(dec_mean_est)), alpha = 0.5) +
  facet_wrap(~decade) +
  theme_sleek(base_size = 6) +
  geom_sf() +
...
# fr_tot ~ 0 + s(doy_sc, bs = "cc") + depth_sc, time_varying = ~ 1 + pred_length_sc,
plot_map_fc +
  geom_point(data = pred_Mod_st$data, aes(X*1000, Y*1000, color = exp(dec_mean_enrf)), alpha = 0.5) +
  facet_wrap(~decade) +
  theme_sleek(base_size = 6) +
  geom_sf() +
...
plot_map_fc +
  geom_point(data = pred_Mod_st$data, aes(X*1000, Y*1000, color = exp(dec_mean_erf)), alpha = 0.5) +
  facet_wrap(~decade) +
  theme_sleek(base_size = 6) +
  geom_sf() +
  scale_fill_viridis(trans = "sqrt") +
...
plot_map_fc +
  geom_point(data = pred_Mod_st$data, aes(X*1000, Y*1000, color = dec_mean_eps), alpha = 0.5) +
  # geom_raster(data = pred_Mod$data |> filter(year == 1993), aes(X*1000, Y*1000, color = omega_s), alpha = 0.5) +
  facet_wrap(~decade) +
  theme_sleek(base_size = 6) +
  geom_sf() +
  scale_fill_viridis(trans = "sqrt") +
