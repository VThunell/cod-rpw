...
df_tot <- df |> 
  filter( fr_tot > 0 & fr_tot < q99,
         !is.na(day_of_year)) |> # should tehse get the mean day_of_year instead?
 mutate(fr_tot_sqrt = sqrt(fr_tot),
        decade = round(year/10) * 10)

hist(df_tot$pred_length)
```
...
# there is something with 1965 making high estimates of fr (~8) for spatial and spattemp random fields. See maps
df_tot |>
  filter(year == 1965) |>
  ggplot(aes(pred_length, fr_tot, color = year))  +
  geom_point() +
...
Mod <- readRDS(Mod, file = "fr-tot_Mod_ar1_sq2024-04-25.rds")
...
bind_rows(index_n, index_s, index_st)  |> 
  ggplot(aes(year, est^2, color = model, fill = model)) +
  geom_line() +
  geom_ribbon(aes(ymin = lwr^2, ymax = upr^2), color = NA, alpha = 0.3) +
  scale_x_continuous(breaks = seq(min(df_tot$year), max(df_tot$year), by = 10)) +
  ylab("index mean") 
...
d_stats <- read_csv(paste0(home, "/R/main-analysis/data_stats.csv"))
...
cpue_cod <- exp(predict(m3, newdata = nd_pred_grid_m3)$est)
...
index_avg_uw <- get_index(pred_avg, area = 1/ncells)

# plot biomass weighted and unweighted index
bind_rows(index_avg |> mutate(type = "w"), index_avg_uw |> mutate(type = "uw")) |>
  ggplot(aes(year, est^2, ymin = lwr^2, ymax = upr^2, color = type, fill = type, linetype = )) +
...
for(i in unique(nd_pred_grid_Mod$sub_div)) {
  ...
  pred_avg_temp <- predict(Mod_s, newdata = temp_pred, return_tmb_object = TRUE)#, nsim = 10)
