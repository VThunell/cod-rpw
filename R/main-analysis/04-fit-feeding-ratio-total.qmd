...
# Mod <- readRDS(Mod, file = paste0("fr-tot_Mod_ar12024-03-13.rds"))
...
index <- lapply(unique(nd_pred_grid$year), \(y) {
  #nd <- replicate_df(pred_grid, "year", y)
  p <- predict(Mod, newdata = nd_pred_grid, return_tmb_object = TRUE)
  get_index(p, area = rep(1/ncells, nrow(p$data)), bias_correct = TRUE)
})
