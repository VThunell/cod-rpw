...
rm(plot_map,plot_map_fc,plot_map_labels,plot_map_labels_fc,Mod_g,Mod_l,df,swe_coast,swe_coast_proj,res_g,res_l,map_data)
gc()
time <- Sys.time()
Mod_full<-
  sdmTMB(
...
saveRDS(Mod_full, file = paste0(home, "/R/main-analysis/rpw-pg_Mod_full_JULY2025",Sys.Date(),".rds"))
# 2 dim
# time <- Sys.time()
# Mod_2d_test <-
#   sdmTMB(
...
sanity(Mod_full)
# predict on data
nd_Mod_full <- expand.grid("pred_length_sc" = seq(min(Mod_full$data$pred_length_sc),
                                                   max(Mod_full$data$pred_length_sc),
                                                   length.out =30), 
                            "prey_group_f" = levels(Mod_full$data$prey_group_f),
                            "year_sc" = seq(min(Mod_full$data$year_sc),
                                            max(Mod_full$data$year_sc),
                                            length.out = length(unique(Mod_full$data$year)))) 

nd_Mod_full <- nd_Mod_full |>
  mutate(depth_sc = 0,
         year = 1993,
         doy_sc = 0,
         depth_sc = 0,
...
gc()
time <- Sys.time()
preds_full <- predict(Mod_full, newdata = nd_Mod_full, re_form = NA)
Sys.time() - time
...
# 1d
mfull_est1 <- tidy(Mod_full, effects = "fixed", conf.int = TRUE)  %>%
...
preds_fullb <- preds_full |>
  mutate(predd = exp(est1+est2),
         norm = (predd)/max(predd), .by = prey_group_f,
         year = year_sc*sd(df_tot$year) + mean(df_tot$year))
preds2b <- preds2 |>
  mutate(predd = exp(est1+est2),
         norm = (predd)/max(predd), .by = prey_group_f,
...
mfull_full1 <- bind_rows(tidy(Mod_full, effects = "ran_par", model = 1, conf.int = TRUE) %>%
                         filter(term %in% c("sigma_O", "sigma_E")),
                       tidy(Mod_full, effects = "fixed", model = 1, conf.int = TRUE)  %>%
                         filter(!grepl('year', term))) %>%

  mutate(term = factor(term))

# Sort the terms so that random effects are at the top...
mfull_full1 <- mfull_full1 %>%
