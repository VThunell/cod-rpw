...
df_tot_env <- df_tot |>
  ...
  mutate(fr_tot_sqrt = sqrt(fr_tot),
         ...
         density_cod_sc = (density_cod - mean(density_cod, na.rm = TRUE))/sd(density_cod, na.rm = TRUE) )
...
#df_long <- df_tot |>
  ...
  filter(!prey_group == "fr_other") |> # needed to avoid ln_smooth_sigma too large and also not of interest
...
Mod_res2 <- residuals(Mod_2denv, model = 2)
Mod_res1 <- residuals(Mod_2denv, model = 1)
...
tidy(Mod_2denv, effects = "ran_par", model = 2, conf.int = TRUE)
...
nd_Mod2d_env <- expand.grid("pred_length_sc" = seq(min(Mod_2denv$data$pred_length_sc),
                            ...
                            "year_sc" = seq(min(Mod_2denv$data$year_sc),
                                            ...
                                            length.out = length(unique(Mod_2denv$data$year))+1 ))#, "year" = sort(unique(Mod_2denv$data$year)))
...
pred2d = predict(Mod_2denv, newdata = nd_Mod2d_env, re_form = NA)
...
pred2d3 |>
  ...
  mutate(norm = (predd)/max(predd), .by = prey_group_f) |>
  ggplot() + 
  geom_raster(aes(year, pred_length, fill = norm)) +
...
temp_pred2d <- readRDS(file = paste0(home, "/R/main-analysis/fr-pg_temp_pred2d2024-06-20.rds"))
...
plotdf <- temp_pred2d$data |>
  ...
  mutate(dec_mean_eps = mean(epsilon_st1+epsilon_st2, na.omit = TRUE), .by = c(decade, prey_group_f, X, Y)) #random spatial and spatiotemporal
         
sum(is.na(plotdf$dec_mean_eps))
plot_map_fc +
  #geom_raster(data = plotdf, aes(X*1000, Y*1000, fill = exp(est1+est2))) +
  geom_raster(data = plotdf, aes(X*1000, Y*1000, fill = dec_mean_eps)) +
  facet_grid(~decade) +#, labeller = labeller(prey_group_f = preygroups)) +
