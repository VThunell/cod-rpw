---
title: "Prey specific feeding ratio fit sdmTMB model"
...
df <- read_csv(paste0(home, "/data/clean/stomachs2.csv")) |>
...
sad_Mod_n3_ntw <- # deltagamma year_f*pred_length_sc
  sdmTMB(
    data = df_prs,
...
sad_Mod_n3_n <- # deltagamma year_f*pred_length_sc
  sdmTMB(
    data = df_prs,
    formula = fr_sad ~ 0 + factor(year)*pred_length_sc + s(doy_sc, bs = "cc") + depth_sc, 
    mesh = mesh_prs,
...
sanity(sad_Mod_n3_ntw)
sanity(sad_Mod_n3_n)
sanity(sad_Mod_n3_s)
sanity(sad_Mod_n3_st)
...
r_df <- bind_rows(tibble(res = residuals(sad_Mod_n3_ntw, model = 1), mod = "n tweedie"),
                  tibble(res = residuals(sad_Mod_n3_n, model = 1), mod = "n hurdle one"),
                  tibble(res = residuals(sad_Mod_n3_n, model = 2), mod = "n hurdle two"),
                  tibble(res = residuals(sad_Mod_n3_s, model = 1), mod = "s hurdle one"),
                  tibble(res = residuals(sad_Mod_n3_s, model = 2), mod = "s hurdle two"),
                  tibble(res = residuals(sad_Mod_n3_st, model = 1), mod = "st hurdle one"),
                  tibble(res = residuals(sad_Mod_n3_st, model = 2), mod = "st hurdle two"))
