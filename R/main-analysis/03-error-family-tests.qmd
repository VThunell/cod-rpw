---
title: "Testing observation error families and feeding ratio cutoffs"
...
quantile(df_noz$fr_tot, 0.95) 
# # remove feeding ratios above the 95th percentile 
df_95 <- df_noz |>
  filter(fr_tot <= 0.11)

quantile(df_noz$fr_tot, 0.99) 
...
qqnorm(residuals(Mod_n_95, "mle-mcmc", mcmc_samples = res_Mod_n_95), main="mcmc 95")
qqline(residuals(Mod_n_95, "mle-mcmc", mcmc_samples = res_Mod_n_95))
qqnorm(residuals(Mod_n_99), main="regular 99")
qqline(residuals(Mod_n_99))
qqnorm(residuals(Mod_n_99, "mle-mcmc", mcmc_samples = res_Mod_n_99), main="mcmc 95")
qqline(residuals(Mod_n_99, "mle-mcmc", mcmc_samples = res_Mod_n_99))
...
# # remove feeding ratios above the 99th percentile 
df_99 <- df_noz |>
  filter(fr_tot <= 0.37) # this differs from the gamma
...
qqnorm(residuals(Mod_n_99), main="regular 99")
qqline(residuals(Mod_n_99))
qqnorm(residuals(Mod_n_99, "mle-mcmc", mcmc_samples = res_Mod_n_99), main="mcmc 99")
qqline(residuals(Mod_n_99, "mle-mcmc", mcmc_samples = res_Mod_n_99))
```
The log normal makes for a way better fit and again, a higher cutoff seems better than a lower. I switch to log normal and use th 99th percentile (fr cutoff = 0.37). And since so much of the data is included in the model, i dont know if a student with logged data is necessary?
