...
res_gMod_n_95 <- mcmc_res <- residuals(gMod_n_95, type = "mle-mcmc", mcmc_samples =  sdmTMBextra::predict_mle_mcmc(gMod_n_95, mcmc_iter = 201, mcmc_warmup = 200))
res_gMod_n_99 <- mcmc_res <- residuals(gMod_n_99, type = "mle-mcmc", mcmc_samples =  sdmTMBextra::predict_mle_mcmc(gMod_n_99, mcmc_iter = 201, mcmc_warmup = 200))

gr_df <- bind_rows(data_frame(res = residuals(gMod_n_95, "mle-mcmc", mcmc_samples = res_gMod_n_95),  typer = "mcmc_95"),
                  data_frame(res = residuals(gMod_n_99, "mle-mcmc", mcmc_samples = res_gMod_n_99),  typer = "mcmc_99"),
                  data_frame(res = residuals(gMod_n_95),  typer = "reg_95"),
                  data_frame(res = residuals(gMod_n_99),  typer = "reg_99"))
...
res_lnMod_n_95 <- mcmc_res <- residuals(lnMod_n_95, type = "mle-mcmc", mcmc_samples =  sdmTMBextra::predict_mle_mcmc(lnMod_n_95, mcmc_iter = 201, mcmc_warmup = 200))
res_lnMod_n_99 <- mcmc_res <- residuals(lnMod_n_99, type = "mle-mcmc", mcmc_samples =  sdmTMBextra::predict_mle_mcmc(lnMod_n_99, mcmc_iter = 201, mcmc_warmup = 200))

lnr_df <- bind_rows(data_frame(res = residuals(lnMod_n_95, "mle-mcmc", mcmc_samples = res_lnMod_n_95),  typer = "mcmc_95"),
                  data_frame(res = residuals(lnMod_n_99, "mle-mcmc", mcmc_samples = res_lnMod_n_99),  typer = "mcmc_99"),
                  data_frame(res = residuals(lnMod_n_95),  typer = "reg_95"),
                  data_frame(res = residuals(lnMod_n_99),  typer = "reg_99"))


ggplot(lnr_df, aes(sample = res)) +
  stat_qq(shape = 1) +
  stat_qq_line() +
  facet_wrap(~typer) +
...
gsim <- simulate(gMod_n_99, nsim = 1000, type = 'mle-mvn')
lnsim <- simulate(lnMod_n_99, nsim = 1000, type = 'mle-mvn')
ggsim <- simulate(ggMod_n_99, nsim = 1000, type = 'mle-mvn')
# gamma
dharma_residuals(gsim, gMod_n_99)
# lognormal
dharma_residuals(lnsim, lnMod_n_99)
# generalised gamma
dharma_residuals(ggsim, ggMod_n_99)

```
The log normal makes for a way better fit and again, a higher cutoff seems better than a lower. I use log normal and the 99th percentile (fr cutoff = 0.37). gengamma() seems really good if we can compare the dharma residuals and it has lower AIC
