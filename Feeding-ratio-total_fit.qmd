...
# without spatial and spatiotemporal it takes 14 sec. Spatial model takes 20 sec. AR1 spatiotemporal model takes 4 hours.
...
AIC(Mod_s)
AIC(Mod)
#saveRDS(Mod, file = paste0("fr-tot_Mod_ar1",Sys.Date(),".rds"))
#Mod <- readRDS(Mod, file = paste0("fr-tot_Mod_ar1",Sys.Date(),".rds"))
...
time <- Sys.time()
# return_tmb_object = TRUE,  #returns a report from TMB in the output
pred_Mod <- predict(Mod, newdata = nd_pred_grid, return_tmb_object = TRUE) # måste ha TRUE här!!
Sys.time() - time
...
index_tot_sm <- get_index(pred_Mod_sm, area = rep(1/ncells, nrow(pred_Mod$data)), bias_correct = TRUE)
index_tot_la <- get_index(pred_Mod_la, area = rep(1/ncells, nrow(pred_Mod$data)), bias_correct = TRUE)
#index_tot <- get_index(pred_Mod, area = rep(1/ncells, nrow(pred_Mod$data)), bias_correct = TRUE)

bind_rows(index_tot_sm |> mutate(length = "15"),
          index_tot |> mutate(length = "36, mean"),
          index_tot_la |> mutate(length = "55"))  |> 
  ggplot(aes(year, est, color = length)) +
  geom_line() +
  #geom_ribbon(aes(ymin = lwr, ymax = upr, alpha = 0.3)) +
  #scale_fill_manual(values = c("white", "grey10")) +
  scale_x_continuous(breaks = seq(min(index_tot$year), max(index_tot$year), by = 10)) + 
